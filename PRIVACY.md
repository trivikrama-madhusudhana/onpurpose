...
- `contextMenus`: add Turn OnPurpose on/off to the menu shown when you right-click its Chrome toolbar icon. This control does not collect page content or browsing activity.
...
- Content script match `https://www.youtube.com/*`: read supported video cards and current-video metadata, and display the goal bar, reversible relevance labels and reminder popup.

Turn OnPurpose off in its toolbar popup or toolbar icon menu to remove its page UI, restore cards, stop new classification requests and abort requests in progress. Requests already sent may still incur provider charges. Turning it off preserves your settings and saved videos. Pause filtering also stops new classification jobs and restores cards while keeping the goal bar available. Clearing or changing the goal prevents old decisions from being applied. The extension never clicks play, changes your YouTube account settings or publishes anything.

## Use of data

OnPurpose uses this data only to provide its visible filtering, reminders, settings and saved-video features. It does not sell user data, use it for advertising, or use it to determine creditworthiness or lending eligibility. OnPurpose complies with the Chrome Web Store User Data Policy, including its Limited Use requirements.
